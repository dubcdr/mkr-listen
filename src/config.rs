...
pub mod config {
    ...
    // Geth Constants
    pub const GETH_HTTP_ENDPOINT: &'static str = "http:H//localhost:8545";
    ...
    pub fn get_config() -> UniListenConfig {
        ...
        let since_block = match matches.value_of("since-block") {
            Some(s) => Some(
                s.parse::<u64>()
        ...
        let prev_blocks = match matches.value_of("prev-blocks") {
            Some(s) => Some(
                s.parse::<u32>()
